...
for _ in range(iters):
    gen.generate(N, random_state=10)
end = time()
    
print(f'Estimation for single core completed. Time spent on one iterration is {(end - start) / iters}')

file = open('sc.txt', 'w')
file.write(f'{(end - start) / iters}')
file.close()